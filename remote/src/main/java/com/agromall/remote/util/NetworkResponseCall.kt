    ...
    override fun enqueue(callback: Callback<NetworkResponse<S, E>>) {
        return delegate.enqueue(object : Callback<S> {
            override fun onResponse(call: Call<S>, response: Response<S>) {
                ...
                var errorMessage = ""
                if (error != null && error.string().isNotEmpty()) {
                    val jsonObject = JsonParser().parse(error?.string()).getAsJsonObject()
                ...
                if (response.isSuccessful) {
                    if (body != null) {
                        ...
                        callback.onResponse(
                            this@NetworkResponseCall,
                            Response.success(NetworkResponse.UnknownError(Throwable(errorMessage)))
                        )
                    }
                } else {
                    val errorBody = when {
